...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<h1 class="text-2xl font-bold text-white">Bon retour</h1><p class="text-sm text-surface-400 mt-2">Connectez-vous \xE0 votre coffre-fort</p></div><form class="space-y-4"><div><label for="email" class="block text-sm font-medium text-surface-300 mb-1">Email</label><input id="email"${ssrRenderAttr("value", unref(form).email)} type="email" required class="input-field" placeholder="vous@exemple.com"></div><div><label for="password" class="block text-sm font-medium text-surface-300 mb-1">Mot de passe</label><input id="password"${ssrRenderAttr("value", unref(form).password)} type="password" required class="input-field" placeholder="\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022"></div>`);
      if (unref(errorMsg)) {
      ...
      _push(`</button></form><p class="text-center text-sm text-surface-400"> Pas encore de compte ? `);
      _push(ssrRenderComponent(_component_NuxtLink, {
        to: "/auth/register",
...
//# sourceMappingURL=login-BO2CHUN2.mjs.map
